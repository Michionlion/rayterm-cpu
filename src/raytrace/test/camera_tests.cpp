...
TEST(Camera, ToWorldDirDifferentOrigin) {
    ...
    // default look target is (0, 0, -1)
    cam->position(vector(1, 0, 0));
    actual   = cam->toWorldDir(vector(0, 0, -1));
    expected = vector(-1, 0, -1).normalized();
    EXPECT_TRUE(actual.isApprox(expected, 0.0001))
        << "  Actual: "
        << "{" << actual[0] << ", " << actual[1] << ", " << actual[2] << "}"
        << "\nExpected: "
    ...
    // FIXME: is this a bug? Should expected direction be (0, 0, 0)?
    // Is this gimbal lock?
    cam->look(vector(1, 0, 0));
    actual   = cam->toWorldDir(vector(0, 0, -1));
    ...
    cam->look(vector(0, 3, -3));
    actual   = cam->toWorldDir(vector(0, 0, -1));
    expected = vector(-1, 3, -3).normalized();
    EXPECT_TRUE(actual.isApprox(expected, 0.0001))
        << "  Actual: "
        << "{" << actual[0] << ", " << actual[1] << ", " << actual[2] << "}"
        << "\nExpected: "
...
TEST(Camera, ToWorldPos) {
    ...
    actual   = cam->toWorldPos(vector(1, 2, 3));
    expected = vector(1, 2, 3);
    EXPECT_TRUE(actual.isApprox(expected, 0.0001))
        << "  Actual: "
        << "{" << actual[0] << ", " << actual[1] << ", " << actual[2] << "}"
